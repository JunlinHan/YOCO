...
parser.add_argument('-e2', '--evaluate2', dest='evaluate2', action='store_true',
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.evaluate2:
        validate2(val_loader2, model, criterion, args)
        return
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    # change augmentations here, this is an example using horizontal flip
...
# partial image recognition (non-overlapping 4 crop evaluate)
def validate2(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            if torch.cuda.is_available():
                target = target.cuda(args.gpu, non_blocking=True)
            img1 = images[:, :, 0:224, 0:224]
            img2 = images[:, :, 0:224, 224:448]
            img3 = images[:, :, 224:448, 0:224]
            img4 = images[:, :, 224:448, 224:448]
            ...
            loss4 = criterion(output4, target)
            loss = (loss1 + loss2 + loss3 + loss4) / 4

            acc1_1, acc5_1 = accuracy(output1, target, topk=(1, 5))
            acc1_2, acc5_2 = accuracy(output2, target, topk=(1, 5))
            acc1_3, acc5_3 = accuracy(output3, target, topk=(1, 5))
            acc1_4, acc5_4 = accuracy(output4, target, topk=(1, 5))

            losses.update(loss.item(), images.size(0))
            acc1 = (acc1_1 + acc1_2 + acc1_3 + acc1_4) / 4
            acc5 = (acc5_1 + acc5_2 + acc5_3 + acc5_4) / 4
            top1.update(acc1[0], images.size(0))
            top5.update(acc5[0], images.size(0))
